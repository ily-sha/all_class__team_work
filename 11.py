    ...
    def draw(self, screen):
        ...
        if index_view:
            pygame.draw.rect(screen, pygame.Color(
                173, 255, 47), (20, 50, 20, 20))
        else:
            pygame.draw.rect(screen, pygame.Color(
                204, 188, 188), (20, 50, 20, 20))
        pygame.draw.line(screen, pygame.Color(0, 0, 0), (29, 53), (29, 67), 2)
        pygame.draw.line(screen, pygame.Color(0, 0, 0), (23, 59), (37, 59), 2)


def get_map(lon, lat, pin=False, pin_lon=None, pin_lat=None):
    global map_file
    params = {
    ...
    if pin:
        params["pt"] = f"{','.join(map(str, [pin_lon, pin_lat]))},pm2rdm"
...
def address_change():
    if index_view:
        try:
            index_pos = toponym["metaDataProperty"]["GeocoderMetaData"]["Address"]["postal_code"] + ", "
        except KeyError:
            index_pos = ""
    else:
        index_pos = ""
    address = index_pos + toponym["metaDataProperty"]["GeocoderMetaData"]["text"][8:]
    input_box.text = f"{address[:63]}..."
    input_box.text_render = input_box.font.render(
        f"{address[:63]}...", True, pygame.Color(0, 0, 0))


def get_coord(x, y):
    return longitude + (x - 300) / 300 * 0.02, latitude - (y - 225) / 225 * 0.02
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT:
                ...
                latitude += 0.02
            get_map(longitude, latitude)
        if event.type == pygame.MOUSEBUTTONDOWN and event.pos[0] in range(20, 40) and event.pos[1] in range(50, 70):
            index_view = not (index_view)
            if index_view:
                pygame.draw.rect(screen, pygame.Color(
                    173, 255, 47), (20, 50, 20, 20))
            else:
        ...
        if (event.type == pygame.MOUSEBUTTONDOWN
            ...
            address_change()
            get_map(longitude, latitude, pin=True, pin_lon=longitude, pin_lat=latitude)
        elif event.type == pygame.MOUSEBUTTONDOWN and event.pos[0] in range(570, 590) and event.pos[1] in range(50, 70):
            input_box.text = ""
            input_box.text_render = input_box.font.render(
            ...
            address_change()
            get_map(longitude, latitude, pin=True, pin_lon=lon, pin_lat=lat)
